const { move, getFilesDeep } = require("..")
const config = require("./setup-and-teardown.js")
const fs = require("fs")
const makeKey = require("./make-key.js")
const path = require("path")

test("tests that files can be moved asynchronously", async () => {
  config.setup()
...
test("tests that directories can be moved asynchronously", async () => {
  config.setup()
  ...
  const originalSrcFiles = await getFilesDeep(src)
...
test("tests that symlink targets are preserved when moved asynchronously", () => {
  // console.warn("Remember to build this test!")
})

test("tests that preexisting files are overwritten during an asynchronous move", () => {
  // console.warn("Remember to build this test!")
})

test("tests that empty directories can be moved asynchronously", () => {
  // console.warn("Remember to build this test!")
})

test("tests that an error is thrown when trying to asynchronously move a directory into itself", () => {
  // console.warn("Remember to build this test!")
})
